    ...
    try:
        ...
        sql_query = """
            ...
            WHERE g.foracid = %s
              ...
              AND g.del_flg = 'N'
           GROUP BY g.cif_id, g.foracid, g.acct_name, g.acct_opn_date, g.sol_id, s.sol_desc, t.deposit_amount;
        """
...
def create_share_transfer(shareholder_cif, deposit_amount, sol_id=None, account_number=None, acct_opn_date=None):
    try:
        ...
        # ✅ Use acct_opn_date from SQL if available
        if acct_opn_date:
            share_transfer_doc.date = acct_opn_date.strftime("%Y-%m-%d")
        else:
            share_transfer_doc.date = datetime.now().strftime("%Y-%m-%d")
...
def create_shareholder_record(account_number):
    try:
        ...
        if not shareholder_data or len(shareholder_data) == 0:
            return {"success": False, "error": "No shareholder data found for this account number."}

        data = shareholder_data[0]
        cif_id = data.get("cif_id")
        acct_opn_date = data.get("acct_opn_date")  # ✅ fetch date from SQL

        if not cif_id:
            return {"success": False, "error": "CIF not found in external database for this account number."}
        ...
        share_transfer_result = create_share_transfer(
            ...
            account_number=account_number,
            acct_opn_date=acct_opn_date  # ✅ pass SQL date here
        )
        ...
        if share_transfer_result.get("success"):
            ...
            frappe.db.rollback()
            return {"success": False, "error": f"Share Transfer creation failed: {share_transfer_result.get('error')}"}

    except Exception as e:
        frappe.db.rollback()
        frappe.log_error(frappe.get_traceback(), "Shareholder Creation Error")
        return {"success": False, "error": f"Error creating shareholder/share transfer record: {str(e)}"}
...
def get_current_series_info():
    try:
        ...
        return {"success": False, "error": f"Error getting series info: {str(e)}"}
