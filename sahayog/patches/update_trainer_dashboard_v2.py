...
def execute():
  ...
  <div class="aad-stats">
    ...
    <div class="aad-stat-card" data-filter="pending">
      <div class="aad-stat-label">Pending</div>
      <div class="aad-stat-value aad-stat-gray aad-pending-count">0</div>
    </div>
  </div>
  ...
  function getFilters(search = "") {
    ...
    if (statusFilter === "stay")         filters["wants_to_stay"] = 1;
      ...
      filters["wants_to_stay"] = 0;
      filters["exited"] = 0;
      filters["reply_type"] = ["not in", ["Follow-up Required", "Call Back Later", "Not Reachable"]];
    }
    return filters;
  }
  ...
  async function fetchAnalytics() {
    ...
    const pending = Math.max(0, total - stay - exited - followup - notreachable);
    ...
    root.querySelector(".aad-pending-count").textContent = pending;
  ...
  function renderTable(logs) {
    ...
    tbody.innerHTML = logs.map((log, i) => {
      ...
      const followupDate = log.follow_up_date ? frappe.datetime.str_to_user(log.follow_up_date) : "-";
  ...
  loadData();
    ...
    doc.style = doc.style.replace(
        ".aad-stat-red { color: var(--danger); }",
    ...
    ) if ".aad-stat-blue" not in (doc.style or "") else doc.style

    # Ensure blue badge exists in style
    if ".aad-badge-blue" not in (doc.style or ""):
        doc.style = (doc.style or "") + "\n.aad-stat-blue { color: #2563eb; }\n.aad-stat-gray { color: #6b7280; }\n.aad-badge-blue { background:#dbeafe; color:#1d4ed8; border:1px solid #93c5fd; }"

    doc.save()
    frappe.db.commit()
    print("Trainer Dashboard updated successfully.")
