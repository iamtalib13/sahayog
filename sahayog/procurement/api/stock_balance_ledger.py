...
def get_asset_list(limit=20, start=0, search_text=None):
    ...
    if search_text:
        conditions.append("(ast.name LIKE %(search)s OR ast.asset_name LIKE %(search)s OR ast.serial_no LIKE %(search)s OR ast.location LIKE %(search)s OR emp.employee_name LIKE %(search)s)")
