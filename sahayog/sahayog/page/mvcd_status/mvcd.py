...
def get_mvcd_status(force=False):
    """
    Fetch Pending Transactions where clr_bal_amt != 0 and bacid in specific list (GAM table)
    caching results to avoid database connection overload.
    """
    try:
        cache_key = "mvcd_status_gam_data"
        ...
        frappe.cache().set_value(cache_key, rows, expires_in_sec=10)
...
@frappe.whitelist(allow_guest=False)
def get_pending_transactions(tran_date=None, force=False):
    """
    Fetch MVCD Status for the given date (default: today) from DTD table,
    caching results to avoid database connection overload.
    """
    try:
        if not tran_date:
        ...
        cache_key = f"mvcd_pending_transactions:{tran_date}"
