...
def execute():
    ...
    js_content = """ 
frappe.call({
    method: "sahayog.scrm.api.sahayog_crm_dashboard.get_zone_region_data",
    callback: function(r) {
...
"""
    ...
    custom_block = frappe.db.exists('Custom HTML Block', 'CRM-Zone and Region wise Data')
    if custom_block:
        doc = frappe.get_doc('Custom HTML Block', 'CRM-Zone and Region wise Data')
        doc.html = html_content
        doc.style = css_content
        doc.script = js_content
        doc.save()
        print("Updated Custom HTML Block: CRM-Zone and Region wise Data")
    else:
        frappe.get_doc({
            'doctype': 'Custom HTML Block',
            'name': 'CRM-Zone and Region wise Data',
            'html': html_content,
            'style': css_content,
            'script': js_content
        }).insert()
        print("Created Custom HTML Block: CRM-Zone and Region wise Data")
